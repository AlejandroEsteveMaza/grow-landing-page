...
const landingPage = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'sectionCopy', title: 'Textos de secciones', type: 'object', fields: ['services', 'process', 'team', 'pricing', 'resources', 'faq'].map((name) => defineField({ name, title: name, type: 'sectionCopy', validation: (rule) => rule.required() })), validation: (rule) => rule.required() }),
